...
\end{comment}
\todo{Many of the problems have too many hints. We should throw away the hints and suggestions which are too trivial for MC students, it's better to have them discuss and struggle in class. We should also have some hard optional problems (like the $ \Z/p$ polynomial counting) which students can try to solve in TAU.}
...
\label{thm:discriminant}
	...
	\end{align*}
\end{thm}
\todo{Is there a way to motivate this? Or have an \emph{elegant} proof which is something less that brute verification.}
...
\todo{possibly add subsection on counting number of squarefree polynomials over $Z/p$ it's a fun motivic induction on the number of square factors}
\todo{maybe also discuss number of pairs of polynomials with a common root, the formula being the same is no coincidence}
