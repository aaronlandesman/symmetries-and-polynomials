...
\begin{exercise}
	...
	\todo{Apurva: It's a little hard to see that guess that every reflection is of the form $rs^k$. }
\end{exercise}
\begin{remark}
	\label{remark:}
...
maybe we can get them there quicker by moving 5.2 to an appendix or something}
	...
	\begin{exercise}
		...
		For $ n=3$, by comparing coefficients of $ x_i$ $$x^3 + a_2 x^2 + a_1 x + a_0 = P(x) = (x-r_1)(x-r_2)(x-r_3)$$ find $ a_i$ in as a function of the $ r_i$.
		Similar identities hold for all $ n$. 
		\todo{Apurva: How about now?}
	\end{exercise}
	Note that each $ a_i$ is a (multi-variable) polynomial in the roots $ r_i$. These polynomials are called the \textbf{elementary symmetric polynomials}. They are called `symmetric' because they remain unchanged upon permuting the roots $ r_i$.
	...
	\end{theorem}
	\begin{remark}
		You can assume this theorem without proof. The proof of this theorem, which is a careful application of the multinomial theorem and some multi-index induction, also gives an algorithm for finding what these polynomials are.
	\end{remark}
	\begin{exercise}
    ...
    \mu_2 &= r_1 + r_2 \omega^2 + r_3 \omega
  \end{align*}
	We've already seen that $ \mu_1^3 + \mu_2^3 = -27q$ and $ \mu_1^3 \mu_2^3 = -27p$, both are symmetric in the variables $ r_1, r_2, r_3$.
	\end{remark}
	...
	Hopefully now you're convinced that there was some reason behind choosing these intermediate variables. Which brings us to the question: Do such variables exist for all degrees? \todo{Apurva: Needs rephrasing.}
	
  \todo{Apurva: Simplifying section 5.2. How about this?}
    ...
    % 
		...
		\begin{exercise}
			...
			\end{enumerate}
			We say that the $ \lambda_i$ are \textbf{fixed} by $K_4$.
		\end{exercise}
