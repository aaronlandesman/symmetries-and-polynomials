...
In this class we'll learn how to solve a cubic and to some extent a quartic. We'll explore the connections between these solutions and group theory and towards the end hint towards the deep connection between polynomials and groups via Galois Theory. This connection allows one to convert a classical question of whether a general quintic can be solved by radicals to a group theory problem, which can be relatively easily answered. 

The 5 day outline is as follows: First two days are dedicated to studying polynomials. For fun, we'll start with an interesting invariant of polynomials called the discriminant and move on to solving the cubic. The next two days are an introduction to group theory. We'll learn about groups as symmetries of geometric objects and then learn about their commutator subgroups. Finally, on the last day we'll connect the two theories and explain the Galois correspondence for the cubic and the quartic. There are plenty of optional problems along the way for those who wish to explore more.
\todo{Apurva: Please feel free to edit this.}


\section{The Discriminant}
\todo{Apurva: Some introduction here.}
...
\subsection{Further questions on discriminants}
...
\subsection{Further questions on symmetry groups}
\begin{exercise}[Optional 1]
	\label{exercise:cube}
...
% \todo{aaron: i think this section should be reworked, because this is the last day so people won't work on it for homework. also, many people
% likely won't get through all the exercises, and so they likely won't reach your description of the galois correspondence,
% Since i think we want everyone to get to the galois correspondence idea, we should streamline what preceeds it as much as possible, and then possibly
% add some exercises after it in case they get through it quickly,
% maybe we can get them there quicker by moving 5.2 to an appendix or something}
...
\begin{exercise}
	...
	This is denoted $K_4$ and is called the {\bf Klein-$4$ group} (in an appropriate sense, it is the same as the klein-4 group defined in \autoref{exercise:klein-4}.
\end{exercise}
...
\begin{exercise}
	\label{exercise:}**
	Show that if $g\in S_n$ can be written as products of transpositions in two ways, say $g = t_1 \cdots t_n$ and $g = r_1 \cdots r_m$ for $t_i, r_j$
	transpositions, then $n \equiv m \bmod 2$.
	\hint{This is a bit tricky, and there are many ways to do it. Here is one nice one:
	Let $\sigma \in S_n$ be some permutation.
	Draw a picture with $1, 2, \cdots, n$ listed at the top and the bottom. Draw the lines connecting $i$ to $\sigma(i)$ so that
	the lines always intersect transversely (i.e., no two lines are tangent at an intersection point).
	Check that the number of intersections of lines is well defined modulo $2$. 
	Using well definedness, check that
...
\begin{exercise}
	...
	\begin{enumerate}
		\item Show that $\left[ S_n, S_n \right] \subset A_n$.
