...
\begin{exercise}
	\label{exercise:}
	A regular $n$-gon in $\br^2$ has $2n$ automorphisms, what are they? \hint{There are $n$ rotations by $(k * 360/n)\degree$ for $0 \leq k \leq n-1$ and $n$ reflections.} This set of automorphims is called the {\bf dihedral group of size $2n$}, denote $D_{2n}$.
\end{exercise}
\begin{exercise}
	\label{exercise:}
...
\todo{Apurva: Reprase? Since you `defined' a group above?}
\begin{exercise}
	\label{exercise:}
...
\begin{exercise}[Challenge 1]
	\label{exercise:cube}
	Identify the group of automorphims of a cube with $S_4$. What is the subgroup of rotations inside this?
	Do the same for the octahedron.
	\todo{Apurva:The rotations of the cube are easy to enumerate, it's a lot of fun to write them down. We can put them as a separate problem before this.}
